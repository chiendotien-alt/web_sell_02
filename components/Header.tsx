...
export default function Header() {
  ...
  return (
    <header className="bg-white sticky top-0 z-40 border-b border-gray-100 print:hidden">
      <div className="max-w-6xl mx-auto px-4 py-3.5 flex items-center justify-between gap-4">
        <Link href="/" className="flex items-center gap-2.5 group">
          <span className="w-9 h-9 rounded-full bg-brand flex items-center justify-center text-white text-base font-serif font-semibold shrink-0">
            {shopName.trim().charAt(0).toUpperCase() || "S"}
          </span>
          <span className="text-xl font-serif font-semibold tracking-tight text-ink whitespace-nowrap">
            {shopName}
          </span>
        </Link>
